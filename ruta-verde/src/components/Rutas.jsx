...
export default function Rutas({ searchData, onStartRoute }) {
  ...
  const sortedRoutes = [...routes].sort((a, b) => {
    ...
    if (filter === 'barato') return parseInt(a.price.replace('$','').replace('.','')) - parseInt(b.price.replace('$','').replace('.',''));
    if (filter === 'verde') return parseFloat(a.co2) - parseFloat(b.co2);
    return 0;
  });

  return (
    <div className="flex flex-col h-full bg-surface animate-fade-in relative">
      {/* HEADER BUSQUEDA */}
      <div className="bg-white p-6 pb-4 shadow-sm relative z-40">
        <div className="flex items-center gap-4 mb-6">
          <button onClick={() => setShowResults(false)} className="p-2 -ml-2 text-secondary active:scale-90 transition-all">
            <ArrowLeft size={24} />
          </button>
          <h2 className="text-xl font-black text-secondary">Planifica tu ruta</h2>
        </div>

        <div className="space-y-3 relative">
          <div className="flex items-center gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-100 focus-within:border-primary/30 transition-all relative">
            ...
            {activeInput === 'from' && suggestions.length > 0 && (
              <div className="absolute top-full left-0 right-0 bg-white border border-gray-100 rounded-2xl shadow-2xl mt-2 z-50 overflow-hidden">
                {suggestions.map(s => (
                  <button key={s} onClick={() => selectSuggestion(s)} className="w-full text-left p-4 hover:bg-gray-50 text-sm font-bold border-b border-gray-50 last:border-0">{s}</button>
                ))}
              </div>
            )}
          </div>
          ...
          {[
            ...
            <button
              ...
              className={`px-4 py-2 rounded-full text-xs font-black whitespace-nowrap transition-all border ${
                filter === f.id ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20 scale-105' : 'bg-white text-gray-400 border-gray-100'
              }`}
            >
              {f.label}
            </button>
      ...
      {/* MAPA SIMULADO */}
      <div className="flex-1 relative overflow-hidden bg-[#e8f4f8]">
        ...
        {/* LOADING SKELETON */}
        {loading && (
          <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-50 p-6 space-y-4">
            <div className="w-full h-24 bg-gray-100 rounded-3xl animate-pulse" />
            <div className="w-full h-24 bg-gray-100 rounded-3xl animate-pulse" />
            <div className="w-full h-24 bg-gray-100 rounded-3xl animate-pulse" />
          </div>
        )}
        ...
        {showResults && !loading && (
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-[40px] shadow-2xl p-6 pb-28 max-h-[70%] overflow-y-auto z-40">
            <div className="w-12 h-1.5 bg-gray-200 rounded-full mx-auto mb-6" />
            <h3 className="text-lg font-black text-secondary mb-4 px-2">Rutas encontradas</h3>
            ...
            <div className="space-y-4">
              {sortedRoutes.map(route => (
                <button
                  ...
                  className={`w-full text-left p-4 rounded-3xl border-2 transition-all active:scale-[0.98] relative overflow-hidden ${
                    selectedRoute === route.id ? 'border-primary bg-primary/5 shadow-lg shadow-primary/5' : 'border-gray-50 bg-white'
                  }`}
                >
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        <span className="text-[10px] font-black text-secondary/60 uppercase tracking-widest">{route.label}</span>
                      ...
                      <div className="flex items-center gap-1 mt-1">
                        {route.icons.map((icon, i) => (
                          <React.Fragment key={i}>
                            ...
                            {i < route.icons.length - 1 && <span className="text-[10px] text-gray-300">→</span>}
                          </React.Fragment>
                        ))}
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-black text-secondary leading-none">{route.time}</p>
            ...
            <div className="mt-6 bg-surface p-4 rounded-[32px] border border-primary/10 flex items-center gap-4">
              <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-2xl">🌳</div>
              <div>
                <p className="text-xs font-bold text-secondary">Esta ruta equivale a</p>
                <p className="text-sm font-black text-primary">Plantar 0.3 árboles hoy</p>
              </div>
            </div>

            <button
              onClick={() => onStartRoute(50)}
              className="w-full mt-6 py-5 bg-primary text-white rounded-[32px] font-black text-xl shadow-xl shadow-primary/20 active:scale-95 transition-all flex items-center justify-center gap-3"
            >
              Iniciar esta ruta <Zap size={24} />
            </button>
